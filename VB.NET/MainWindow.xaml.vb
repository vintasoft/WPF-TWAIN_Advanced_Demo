        ...
        Try
            ...
            Try
                ...
                If acquireAllImagesRadioButton.IsChecked = False Then
                    Dim xferCount As Int16 = CType(Math.Truncate(imagesToAcquireNumericUpDown.Value), Int16)

                    ' patch for "bug" in Kodak drivers
                    If device.Info.ProductName.ToUpper().StartsWith("KODAK") Then
                        If device.ShowUI And xferCount = 1 Then
                            xferCount = -1
                        End If
...
#Region "Preview"
    ...
    Private Function GetCurrentImageInfo(ByVal index As Integer, ByVal acquiredImage As AcquiredImage) As String
        ...
        Return String.Format("Image {0} from {1} ({2} x {3}, {4} bpp, {5})", index + 1, _images.Count, imageInfo.Width, imageInfo.Height, imageInfo.BitCount, _
